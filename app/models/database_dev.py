...
class DatabaseDev():
    ...
    def add_new_shift_status_on_firestore(self, new_status, user_id):
        ...
        if turno:
            ...
            self.update_info('Turnos-dev',vars(turno), query_arr=[['dia', turno.dia], ['user_id', turno.user_id]])
        else:
            if new_status == 'clock_in':
                ...
                self.add_data_on_firestore('Turnos-dev', vars(turno))
                
        return True

    def get_turno(self, date, user_id):

        query = self.firestore.collection('Turnos-dev').where(
            'user_id', '==', user_id
        ).where(
            'dia', '==', date
        ).limit(1).stream()
    ...
    def get_cargo(self, cargo_id):
        collection = self.firestore.collection('Cargos-dev')
    ...
    def get_feriados(self):
        ...
        feriados = list()
        query = self.firestore.collection('Feriados-dev').stream()
    ...
    def get_ferias(self, ferias_id):
        stream = self.firestore.collection('Ferias-dev').where(
            'id', '==', ferias_id
        ).stream()
    ...
    def get_all_ferias(self):
        ...
        query = self.firestore.collection('Ferias-dev').stream()
        all_ferias = list()
        for result in query:
